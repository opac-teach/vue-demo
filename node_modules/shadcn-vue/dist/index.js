...
import { existsSync, promises as fs6 } from "node:fs";
    ...
    transform({ scriptASTs, sfcAST, utils: { traverseScriptAST, traverseTemplateAST } }) {
      ...
      for (const scriptAST of scriptASTs) {
        traverseScriptAST(scriptAST, {
          visitLiteral(path17) {
            if (path17.parent.value.type !== "ImportDeclaration" && typeof path17.node.value === "string") {
              path17.node.value = applyColorMapping(path17.node.value.replace(/"/g, ""), baseColor.inlineColors);
              transformCount++;
            }
            return this.traverse(path17);
          }
        });
      }
      if (sfcAST) {
...
function transformImport(opts) {
  return {
    ...
    transform({ scriptASTs, utils: { traverseScriptAST } }) {
      ...
      for (const scriptAST of scriptASTs) {
        traverseScriptAST(scriptAST, {
          visitImportDeclaration(path17) {
            if (typeof path17.node.source.value === "string") {
              const sourcePath = path17.node.source.value;
              if (sourcePath.startsWith("@/registry/")) {
                if (config.aliases.ui) {
                  path17.node.source.value = sourcePath.replace(/^@\/registry\/[^/]+\/ui/, config.aliases.ui);
                } else {
                  path17.node.source.value = sourcePath.replace(/^@\/registry\/[^/]+/, config.aliases.components);
                }
              }
              if (sourcePath === "@/lib/utils") {
                const namedImports = path17.node.specifiers?.map((node) => node.local?.name ?? "") ?? [];
                const cnImport = namedImports.find((i) => i === "cn");
                if (cnImport) {
                  path17.node.source.value = sourcePath.replace(/^@\/lib\/utils/, config.aliases.utils);
                }
              }
            }
            return this.traverse(path17);
      ...
      for (const scriptAST of scriptASTs) {
        traverseScriptAST(scriptAST, {
          visitCallExpression(path17) {
            if (path17.node.callee.type === "Identifier" && path17.node.callee.name === "cva") {
              const nodes = path17.node.arguments;
              nodes.forEach((node) => {
                if (node.type === "Literal" && typeof node.value === "string") {
  ...
  return {
    ...
    transform({ scriptASTs, sfcAST, utils: { traverseScriptAST, traverseTemplateAST } }) {
      ...
      for (const scriptAST of scriptASTs) {
        traverseScriptAST(scriptAST, {
          visitImportDeclaration(path17) {
            if (![ICON_LIBRARIES.radix.import, ICON_LIBRARIES.lucide.import].includes(`${path17.node.source.value}`))
              return this.traverse(path17);
            ...
            if (targetedIconsMap.size > 0)
              path17.node.source.value = ICON_LIBRARIES[targetLibrary].import;
            return this.traverse(path17);
          }
        });
        if (sfcAST) {
...
// src/utils/updaters/update-files.ts
import path8, { basename, dirname } from "pathe";
import prompts from "prompts";
function resolveTargetDir(projectInfo, config, target) {
  if (target.startsWith("~/")) {
    return path8.join(config.resolvedPaths.cwd, target.replace("~/", ""));
  }
  return path8.join(config.resolvedPaths.cwd, target);
}
async function updateFiles(files, config, options) {
  ...
  if (!config.typescript) {
    for (const file of files) {
      ...
      const dirName = path8.dirname(file.path);
      tempRoot = path8.join(tmpdir2(), "shadcn-vue");
      const tempDir = path8.join(tempRoot, "registry", config.style, dirName);
      const tempPath = path8.join(tempRoot, "registry", config.style, file.path);
      await fs6.mkdir(tempDir, { recursive: true });
      await fs6.writeFile(tempPath, file.content, "utf-8");
    }
    await fs6.cp(path8.join(process.cwd(), "node_modules"), tempRoot, { recursive: true });
    await fs6.writeFile(path8.join(tempRoot, "tsconfig.json"), `{
  "compilerOptions": {
    "baseUrl": ".",
    "paths": {
      "@/*": ["./*"]
  ...
  for (const file of files) {
    ...
    if (file.target) {
      ...
      targetDir = path8.dirname(filePath);
    }
    if (!config.typescript) {
    ...
    if (file.type === "registry:ui") {
      ...
      if (folderSkipped.get(folderName) === true) {
        filesSkipped.push(path8.relative(config.resolvedPaths.cwd, filePath));
        continue;
      }
    } else {
    ...
    const content = await transform2({
      filename: path8.join(tempRoot, "registry", config.style, file.path),
      raw: file.content,
      config,
      baseColor
    });
    await fs6.writeFile(filePath, content, "utf-8");
    existingFile ? filesUpdated.push(path8.relative(config.resolvedPaths.cwd, filePath)) : filesCreated.push(path8.relative(config.resolvedPaths.cwd, filePath));
  }
  if (tempRoot) {
    await fs6.rm(tempRoot, { recursive: true });
  }
...
async function updateTailwindContent(content, config, options) {
  ...
  const tailwindFileRelativePath = path9.relative(
    config.resolvedPaths.cwd,
    config.resolvedPaths.tailwindConfig
  );
  const tailwindSpinner = spinner(
  ...
  const raw = await fs7.readFile(config.resolvedPaths.tailwindConfig, "utf8");
  const output = await transformTailwindContent(raw, content, config);
  await fs7.writeFile(config.resolvedPaths.tailwindConfig, output, "utf8");
  tailwindSpinner?.succeed();
}
...
async function runInit(options) {
  ...
  const targetPath = path10.resolve(options.cwd, "components.json");
...
async function preFlightAdd(options) {
  const errors = {};
  if (!fs9.existsSync(options.cwd) || !fs9.existsSync(path11.resolve(options.cwd, "package.json"))) {
    errors[MISSING_DIR_OR_EMPTY_PROJECT] = true;
    return {
      errors,
      config: null
  ...
  if (!fs9.existsSync(path11.resolve(options.cwd, "components.json"))) {
...
import { existsSync as existsSync2, promises as fs10 } from "node:fs";
...
var diff = new Command3().name("diff").description("check for updates against the registry").argument("[component]", "the component name").option("-y, --yes", "skip confirmation prompt.", false).option(
  ...
  try {
    ...
    const cwd = path13.resolve(options.cwd);
    if (!existsSync2(cwd)) {
...
async function diffComponent(component, config) {
  ...
  for (const item of payload) {
    ...
    for (const file of item.files ?? []) {
      ...
      const fileContent = await fs10.readFile(filePath, "utf8");
...
async function migrateIcons(config) {
  ...
  const { confirm } = await prompts4({
    ...
    message: `We will migrate ${highlighter.info(
      ...
      `./${path14.relative(config.resolvedPaths.cwd, uiPath)}`
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      const filePath = path14.join(uiPath, file);
...
async function migrateIconsFile(content, sourceLibrary, targetLibrary, iconsMapping) {
  ...
  const dir = await fs11.mkdtemp(path14.join(tmpdir3(), "shadcn-"));
  const project = new Project2({
    compilerOptions: {}
  });
  const tempFile = path14.join(
    dir,
...
async function preFlightMigrate(options) {
  const errors = {};
  if (!fs12.existsSync(options.cwd) || !fs12.existsSync(path15.resolve(options.cwd, "package.json"))) {
    errors[MISSING_DIR_OR_EMPTY_PROJECT] = true;
    return {
      errors,
      config: null
  ...
  if (!fs12.existsSync(path15.resolve(options.cwd, "components.json"))) {
    errors[MISSING_CONFIG] = true;
    return {
      errors,
      config: null
